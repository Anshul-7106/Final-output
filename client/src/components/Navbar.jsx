...
const Navbar = ({ user, isAdmin, onLogout, searchBar }) => {
  ...
  return (
    <nav className="bg-zinc-950 border-b border-yellow-500/20 sticky top-0 z-40 shadow-md">
          ...
          <div className="flex items-center gap-4 lg:gap-6 xl:gap-10">
            <Link to="/" className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                <span className="block text-2xl lg:text-3xl xl:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-600 tracking-wider leading-none drop-shadow-md">EA</span>
                <span className="block text-[10px] md:text-xs xl:text-sm font-extrabold text-yellow-500/80 tracking-widest mt-0.5 xl:mt-1 uppercase">Encore Ascend</span>
              </div>
            </Link>
            
            <div className="hidden lg:flex items-center gap-4 xl:gap-8 border-l border-zinc-800 pl-4 lg:pl-6 xl:pl-10">
              <Link to="/dashboard" className="text-[15px] font-bold text-zinc-400 hover:text-yellow-500 transition-colors whitespace-nowrap">Resources & Notes</Link>
              <Link to="/video" className="text-[15px] font-bold text-zinc-400 hover:text-yellow-500 transition-colors whitespace-nowrap">Video</Link>
              <Link to="/about" className="text-[15px] font-bold text-zinc-400 hover:text-yellow-500 transition-colors whitespace-nowrap">About Us</Link>
              <Link to="/contact" className="text-[15px] font-bold text-zinc-400 hover:text-yellow-500 transition-colors whitespace-nowrap">Contact</Link>
          ...
          <div className="hidden lg:flex items-center gap-4 xl:gap-6 flex-shrink-0">
            {!user ? (
              ...
              <>
                ...
                <button onClick={onLogout} className="text-sm font-bold text-gray-300 hover:text-white transition-colors border border-gray-700 hover:border-gray-400 bg-gray-800/50 hover:bg-gray-700/50 rounded-lg px-5 py-2">
                  Sign Out
                </button>
              </>
            )}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-zinc-900 border-b border-zinc-800 px-4 pt-2 pb-4 space-y-2">
           <Link to="/dashboard" className="block text-zinc-300 font-bold hover:text-yellow-500 py-2">Resources & Notes</Link>
           ...
           {!user ? (
             ...
             <button onClick={onLogout} className="w-full text-center text-sm font-bold text-gray-300 hover:text-white transition-colors border border-gray-700 bg-gray-800/50 rounded-lg py-2">
                Sign Out
             </button>
           )}
        </div>
